...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { recipientAddress, proofIdentifier, amount = '10' } = body // ✅ Accept custom amount
    ...
    console.log(`🪙 Minting ${amount} GIT to:`, recipientAddress)
    ...
    // Mint tokens with dynamic amount
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        amount: `${amount} GIT`,
        blockNumber: receipt.blockNumber,
        explorerUrl: `https://sepolia.etherscan.org/tx/${tx.hash}`
      }
    })
